...
var dialog_script = [
	...
	#},
	{
		'background': "res://addons/dialogs/Images/background/placeholder-3.png"
	},
	{
		'character': 'Iteb',
		'position': 'center',
...
func _process(delta):
	...
	if Input.is_action_just_pressed(input_next) && Globals.game_paused: 
	...
	match event:
		...
		{'action'}:
			...
			if event['action'] == 'focusout_portraits':
				...
				load_dialog(true)
		{'scene'}:
			Transit.change_scene(event['scene'], 2)
		{'background'}:
